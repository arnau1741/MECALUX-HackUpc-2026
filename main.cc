...
vector<Obstacle> pObs(const string &f) {
  ...
  while (getline(file, l)) {
    stringstream ss(l);
    string x, y, w, d;
    if (getline(ss, x, ',') && getline(ss, y, ',') && getline(ss, w, ',') &&
        getline(ss, d, ','))
...
vector<CeilingRegion> pC(const string &f) {
  vector<CeilingRegion> c;
  ifstream file(f);
  string l;
  while (getline(file, l)) {
    stringstream ss(l);
    string x, h;
    if (getline(ss, x, ',') && getline(ss, h, ','))
      c.push_back({stod(x), stod(h)});
  }
  return c;
}
vector<BayType> pBT(const string &f) {
  ...
  while (getline(file, l)) {
    stringstream ss(l);
    string id, w, d, h, g, nl, pr;
    if (getline(ss, id, ',') && getline(ss, w, ',') && getline(ss, d, ',') &&
        getline(ss, h, ',') && getline(ss, g, ',') && getline(ss, nl, ',') &&
        getline(ss, pr, ','))
...
pair<Box, Box> getB(double x, double y, double w, double d, double g,
                    double r) {
  int rot = (int)r % 360;
  if (rot == 0)
    return {{x, y, w, d}, {x, y + d, w, g}};
  if (rot == 90)
    return {{x, y, d, w}, {x + d, y, g, w}};
  if (rot == 180)
    return {{x, y, w, d}, {x, y - g, w, g}};
  return {{x, y, d, w}, {x - g, y, g, w}}; // 270
}

bool valid(const PlacedBay &nb, const BayType &bt, const WarehouseProblem &prob,
           const vector<PlacedBay> &cur) {
  auto [ph, ga] = getB(nb.x, nb.y, bt.width, bt.depth, bt.gap, nb.rot);
  if (!boxIn(ph, prob.warehousePolygon) || !boxIn(ga, prob.warehousePolygon))
    return false;

  for (const auto &o : prob.obstacles) {
    Box ob = {o.x, o.y, o.width, o.depth};
    if (over(ph, ob) || over(ga, ob))
      return false;
  }

  for (const auto &eb : cur) {
    auto it = find_if(prob.bayTypes.begin(), prob.bayTypes.end(),
                      [&](const BayType &t) { return t.id == eb.id; });
    auto [eP, eG] = getB(eb.x, eb.y, it->width, it->depth, it->gap, eb.rot);
    // Los Gaps sí pueden solaparse entre ellos (comparten pasillo), pero no con
    // el hierro
    if (over(ph, eP) || over(ph, eG) || over(ga, eP))
      return false;
  }
  return true;
}
...
int main() {
  ...
  if (prob.bayTypes.empty())
    return 1;

  vector<BayType> types = prob.bayTypes;
  // Ordenamos por eficiencia para maximizar el retorno de inversión [cite: 138]
  sort(types.begin(), types.end(), [](const BayType &a, const BayType &b) {
    return a.getEff() > b.getEff();
  });

  double mxX = 0, mxY = 0;
  for (auto &p : prob.warehousePolygon) {
    mxX = max(mxX, p.x);
    mxY = max(mxY, p.y);
  }
  ...
  for (double y = 0; y < mxY; y += step) {
    for (double x = 0; x < mxX;) { // Controlamos el avance de x manualmente
      ...
      if (!placed)
        x += step; // Si no cabe nada, avanza un poquito
    }
  }

  for (auto &b : bestBays)
    cout << b.id << ", " << b.x << ", " << b.y << ", " << b.rot << endl;
  return 0;
}